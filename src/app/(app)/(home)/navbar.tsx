  ...
  return (
    <nav className="h-20 flex border-b justify-between font-medium bg-white">
      <Link href="/" className="pl-6 flex items-center">
        <span className={cn("text-5xl font-semibold", poppins.className)}>Ozon clone</span>
      </Link>

      <NavbarSidebar
        onOpenChange={() => setSidebarOpen(value => !value)}
        items={navbarItems}
        open={isSidebarOpen}
      />
      ...
      <div className="hidden lg:flex">
        ...
        >
          <Link href="/sign-in">Залогиниться</Link>
        </Button>
        <Button
          asChild
          variant="secondary"
          className={cn(
            "border-l border-t-0 border-b-0 border-r-0",
            "px-12 h-full rounded-none bg-black",
            "hover:bg-pink-400 transition-colors text-lg",
